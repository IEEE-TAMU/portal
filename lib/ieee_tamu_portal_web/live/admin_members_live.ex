defmodule IeeeTamuPortalWeb.AdminMembersLive do
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      socket
      |> assign(:page_title, "IEEE Admin - Members")
  ...
  def render(assigns) do
    ...
    <.header>
      Members
      <:subtitle>A list of all members in the IEEE TAMU Portal.</:subtitle>
    </.header>

    <div class="mt-6">
      <.filter_form meta={@meta} path={~p"/admin/members"} />
    </div>

    <div class="mt-6">
      <.pagination meta={@meta} path={~p"/admin/members"} />
    </div>
          ...
          <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
            ...
            <%= if Enum.empty?(@members) do %>
              <.empty_state
                icon="hero-users"
                title="No members"
                subtitle="No members match your search criteria."
              />
            <% end %>
          </div>
        </div>
