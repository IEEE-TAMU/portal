defmodule IeeeTamuPortalWeb.MemberSettingsLive do
  ...
  defp show_external_accounts?(assigns) do
    discord_linked?(assigns) or google_linked?(assigns) or
      assigns.discord_oauth_enabled or assigns.google_oauth_enabled
  end
  ...
  def handle_event("unlink_discord", _params, socket) do
    ...
    case Accounts.unlink_auth_method(member, :discord) do
      {:ok, auth_method} ->
        ...
        if IeeeTamuPortal.Features.enabled?(:discord_bot) do
  ...
  def render(assigns) do
    ...
    <div class="space-y-12 divide-y">
      <%= if show_external_accounts?(assigns) do %>
        <div>
